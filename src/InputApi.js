...
export default ({dataset, dispatch}) => {
    ...
    const UpdateSfCrimes = async (startDate, endDate) => {
        ...
        while (fetched !== 0) {
            await fetch(`https://data.sfgov.org/resource/wg3w-h783.json?%24where=((incident_date%20%3E%3D%20%22${startDate}%22)%20and%20(incident_date%20%3C%3D%20%22${endDate}%22))&%24offset=${offset}&%24limit=${limit}`, {method: 'GET'})
        ...
        <Box pad={{top: 'small'}}>
            <Box direction='row'>
                ...
                <DateInput
                    ...
                    onChange={(event) => setDates({...dates, start: event.value})}
                />
            </Box>
            ...
            <Box direction='row'>
                ...
                <DateInput
                    ...
                    onChange={(event) => setDates({...dates, end: event.value})}
                />
            </Box>

            <Box width='100%' pad='small'>
                <Button
                    label={requestBusy ? 'Loading...' : 'Update Map'}
                    disabled={requestBusy}
                    primary={true}
                    onClick={() => setRequestBusy(true)}
                />
